...
const UserFileUploader = () => {
  ...
  const fetchFiles = async () => {
    setLoading(true);
    try {
      const res = await axios.get("/user-files/files", getAuthHeaders());
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post("/user-files/upload", formData, {
        ...getAuthHeaders(),
        headers: {
  ...
  const confirmRename = async () => {
    ...
    setRenaming(true);
    try {
      await axios.put(`/user-files/rename/${fileToRename._id}`, { newName: newFileName }, getAuthHeaders());
  ...
  const confirmDelete = async () => {
    setDeleting(true);
    try {
      await axios.delete(`/user-files/delete/${fileToDelete._id}`, getAuthHeaders());
